...
{
    $storyid = $_POST['id'];
    $editnumber = mysql_num_rows(mysql_query("SELECT * FROM additions WHERE storyid=$storyid AND approved=1"));
...
if(isset($_POST["feature"]))
{
    ...
    if(mysql_num_rows($selectifmod) || mysql_num_rows($selectifstarted))
    {
        mysql_query("UPDATE stories SET featured=1 WHERE id=$storyid") or die(mysql_error());
    }
}
if(isset($_POST["unfeature"]))
{
    ...
    if(mysql_num_rows($selectifmod) || mysql_num_rows($selectifstarted))
    {
        mysql_query("UPDATE stories SET featured=0 WHERE id=$storyid") or die(mysql_error());
    }
}
?>