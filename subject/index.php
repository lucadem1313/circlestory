...
<div id="container">
    <div id="story">
        <?php
            ...
            $selectsubjectid = mysql_query("SELECT * FROM subjects WHERE name='$query'") or die(mysql_error());
            while($row = mysql_fetch_array($selectsubjectid))
            {
                $subjectid = $row{'id'};
            }
            ...
            if($type == "newest")
                $selectsubject = mysql_query("SELECT * FROM storysubjects WHERE subjectid=$subjectid ORDER BY id DESC") or die(mysql_error());
            else if($type == "oldest")
                $selectsubject = mysql_query("SELECT * FROM storysubjects WHERE subjectid=$subjectid ORDER BY id ASC") or die(mysql_error());
            else
                $selectsubject = mysql_query("SELECT * FROM storysubjects WHERE subjectid=$subjectid ORDER BY id ASC") or die(mysql_error());
            
                
            while($row2 = mysql_fetch_array($selectsubject))
            {
                
                if($type == "newest")
                    $selectstory = mysql_query("SELECT * FROM stories WHERE id=".$row2{'storyid'}." ORDER BY id DESC") or die(mysql_error());
                else if($type == "oldest")
                    $selectstory = mysql_query("SELECT * FROM stories WHERE id=".$row2{'storyid'}." ORDER BY id ASC") or die(mysql_error());
                else
                    $selectstory = mysql_query("SELECT * FROM stories WHERE id=".$row2{'storyid'}." ORDER BY id DESC") or die(mysql_error());
            ...
            if(mysql_num_rows($selectsubject) < 1)
            {
